...
const HairCare = () => {
  ...
  // Memoize fetchSkinCareProducts to prevent unnecessary recreations
  const fetchSkinCareProducts = useCallback(async (signal) => {
    setLoading(true);
    setError(null);

    try {
      // First, get all categories to find the Skin Care category
      ...
      if (skinCareCategory) {
        setCategoryId(skinCareCategory._id);

        // Fetch products for this category
        const response = await api.post('/products/filter', {
          categories: [skinCareCategory._id],
          page: 1,
          limit: 8
        }, { signal });
      ...
      // If no skin care category found, don't show the section (products stays empty)
    } catch (err) {
      // Don't set error if request was cancelled
      if (err.name !== 'CanceledError' && err.name !== 'AbortError') {
        console.error('Error fetching skin care products:', err);
        const errorMessage = err.response?.data?.message || err.message || 'Failed to load skin care products';
  ...
  // Fetch skin care products on mount
  useEffect(() => {
    const abortController = new AbortController();

    fetchSkinCareProducts(abortController.signal);
  ...
  if (loading) {
    return (
      <section className="py-8 sm:py-12 bg-white" id="skincare-beauty-section">
        <div className="max-w-[100rem] mx-auto px-4">
          <div className="mb-6 sm:mb-8">
            <h2 className="text-2xl sm:text-3xl font-bold text-[#5c2d16] mb-2">
              Hair Care & Hair Growth 
            </h2>
            <p className="text-sm sm:text-base text-gray-600">Hair Care & Hair Growth - Natural radiance from within</p>
          </div>

          {/* Poster */}
          <div className="mb-6 sm:mb-8 rounded-lg overflow-hidden">
            <img
              src="category/hair_care.jpeg"
              alt="Hair Care & Hair Growth "
  ...
  return (
    <section className="py-8 sm:py-12 bg-white" id="skincare-beauty-section">
      <div className="max-w-[100rem] mx-auto px-4">
        ...
        <div className="skincare-beauty-slider relative">
          <Swiper
            ...
            navigation={{
              nextEl: '.skincare-beauty-swiper-button-next',
              prevEl: '.skincare-beauty-swiper-button-prev',
            }}
            pagination={{
              clickable: true,
              el: '.skincare-beauty-swiper-pagination',
            }}
            autoplay={{
              delay: 5000,
          ...
          {/* Custom Pagination */}
          <div className="skincare-beauty-swiper-pagination flex justify-center mt-4"></div>
        </div>
      ...
      <style dangerouslySetInnerHTML={{
        __html: `
        .skincare-beauty-slider .swiper-slide {
          height: auto;
          display: flex;
        ...
        .skincare-beauty-slider .swiper-slide > div {
          width: 100%;
        }
        .skincare-beauty-slider .swiper-pagination-bullet {
          width: 8px;
          height: 8px;
          background: #d1d5db;
          opacity: 1;
        ...
        .skincare-beauty-slider .swiper-pagination-bullet-active {
          width: 24px;
          border-radius: 4px;
          background: #111827;
        }
        .skincare-beauty-swiper-button-prev.swiper-button-disabled,
        .skincare-beauty-swiper-button-next.swiper-button-disabled {
          opacity: 0.3;
          cursor: not-allowed;
        }
      `}} />
