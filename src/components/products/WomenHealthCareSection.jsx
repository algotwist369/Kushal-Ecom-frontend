...
const WomenHealthCareSection = () => {
  ...
  // Memoize fetchWomenHealthProducts to prevent unnecessary recreations
  const fetchWomenHealthProducts = useCallback(async (signal) => {
    setLoading(true);
    setError(null);

    try {
      // First, get all categories to find the Women Health category
      ...
      // Find category that matches "Women" or "Female" (case insensitive)
      const womenCategory = categories.find(cat =>
        ...
        cat.name.toLowerCase().includes('ladies')
      );

      if (womenCategory) {
        setCategoryId(womenCategory._id);

        // Fetch products for this category
        const response = await api.post('/products/filter', {
          categories: [womenCategory._id],
          page: 1,
          limit: 8
        }, { signal });
      ...
      // If no women category found, don't show the section (products stays empty)
    } catch (err) {
      // Don't set error if request was cancelled
      if (err.name !== 'CanceledError' && err.name !== 'AbortError') {
        console.error('Error fetching women health products:', err);
        const errorMessage = err.response?.data?.message || err.message || 'Failed to load women health products';
  ...
  // Fetch women health products on mount
  useEffect(() => {
    const abortController = new AbortController();

    fetchWomenHealthProducts(abortController.signal);

    // Cleanup: abort pending request on unmount
    return () => {
      abortController.abort();
    };
  }, [fetchWomenHealthProducts]);
  ...
  if (loading) {
    return (
      <section className="py-8 sm:py-12 bg-white" id="women-health-section">
        <div className="max-w-[100rem] mx-auto px-4">
          <div className="mb-6 sm:mb-8">
            <h2 className="text-2xl sm:text-3xl font-bold text-[#5c2d16] mb-2">
              Women Health Care
            </h2>
            <p className="text-sm sm:text-base text-gray-600">Natural wellness solutions for women</p>
          </div>
  ...
  return (
    <section className="py-8 sm:py-12 bg-white" id="women-health-section">
      <div className="max-w-[100rem] mx-auto px-4">
        ...
        <div className="women-health-slider relative">
          <Swiper
            ...
            navigation={{
              nextEl: '.women-health-swiper-button-next',
              prevEl: '.women-health-swiper-button-prev',
            }}
            pagination={{
              clickable: true,
              el: '.women-health-swiper-pagination',
            }}
            autoplay={{
              delay: 5000,
          ...
          {/* Custom Pagination */}
          <div className="women-health-swiper-pagination flex justify-center mt-4"></div>
        </div>
        ...
        {categoryId && (
          <div className="mt-6 sm:mt-8 flex justify-center">
            ...
            >
              View All Women Health Products →
            </button>
          </div>
        )}
      </div>
      ...
      <style dangerouslySetInnerHTML={{
        __html: `
        .women-health-slider .swiper-slide {
          height: auto;
          display: flex;
        ...
        .women-health-slider .swiper-slide > div {
          width: 100%;
        }
        .women-health-slider .swiper-pagination-bullet {
          width: 8px;
          height: 8px;
          background: #d1d5db;
          opacity: 1;
        ...
        .women-health-slider .swiper-pagination-bullet-active {
          width: 24px;
          border-radius: 4px;
          background: #111827;
        }
        .women-health-swiper-button-prev.swiper-button-disabled,
        .women-health-swiper-button-next.swiper-button-disabled {
          opacity: 0.3;
          cursor: not-allowed;
        }
      `}} />
...
export default WomenHealthCareSection;
