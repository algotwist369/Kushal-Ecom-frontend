...
const CategorySection = ({
  ...
  // Memoize fetchCategoryProducts to prevent unnecessary recreations
  const fetchCategoryProducts = useCallback(async (signal) => {
    setLoading(true);
    setError(null);

    try {
      // First, get all categories to find the matching category
      ...
      // Find category that matches any of the provided names (case insensitive)
      const matchedCategory = categories.find(cat =>
        categoryNames.some(name => cat.name.toLowerCase().includes(name.toLowerCase()))
      );
      ...
      if (matchedCategory) {
        ...
        // If no matching category found, fetch some random products
        const response = await api.post('/products/filter', {
          page: 1,
          limit: 8
        }, { signal });
      ...
      if (err.name !== 'CanceledError' && err.name !== 'AbortError') {
        ...
        const errorMessage = err.response?.data?.message || err.message || `Failed to load ${title} products`;
  ...
  // Fetch category products on mount
  useEffect(() => {
    const abortController = new AbortController();

    fetchCategoryProducts(abortController.signal);
  ...
  if (loading) {
    return (
      <section className={`py-8 sm:py-12 ${bgColor}`} id={sectionId}>
  ...
  return (
    <section className={`py-8 sm:py-12 ${bgColor}`} id={sectionId}>
      <div className="max-w-[100rem] mx-auto px-4">
        ...
        <div className={`${sectionId}-slider relative`}>
          <Swiper
          ...
          {/* Custom Pagination */}
          <div className={`${sectionId}-swiper-pagination flex justify-center mt-4`}></div>
        </div>
      ...
      <style dangerouslySetInnerHTML={{
        ...
        .${sectionId}-slider .swiper-slide > div {
          width: 100%;
        }
        .${sectionId}-slider .swiper-pagination-bullet {
          width: 8px;
          height: 8px;
          background: #d1d5db;
          opacity: 1;
        ...
        .${sectionId}-slider .swiper-pagination-bullet-active {
          width: 24px;
          border-radius: 4px;
          background: #111827;
        }
        .${sectionId}-swiper-button-prev.swiper-button-disabled,
        .${sectionId}-swiper-button-next.swiper-button-disabled {
          opacity: 0.3;
          cursor: not-allowed;
        }
      `}} />
